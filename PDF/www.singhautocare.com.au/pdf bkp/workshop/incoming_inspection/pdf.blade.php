...
<table style="width:100%; font-family:lato; font-size:16px;">
  <tbody>
<tr>
      <td style="width:33%; vertical-align: top;"><span ><strong>Booking Time:</strong> {{ date('d-m-y',strtotime($details->booking_date)) }}</span><br />
      ...
      <td style="width:33%; vertical-align: top;"><span ><strong>Name:</strong> {{ ucfirst($details->name) }}</span><br />
...
<th style="text-align:left; padding:5px 10px 10px;">VEHICLE DETAIL</th>
 ...
 <table style="width:100%; font-family:lato; font-size:16px;">
    <tbody>
        <tr>
        <td  style="width:33%; vertical-align: top;"><span ><strong>Rego No:</strong> {{ $details->rego_number }}</span><br />
        ...
        <td  style="width:33%; vertical-align: top;"><span ><strong>Engine No: </strong> {{ $details->engine_number }}</span><br />
...
<td style="width:50%; border: 1px solid #7f7f7f7a; padding:5px;"> {{ucfirst(str_replace('_',' ',$inspection_items_input->value))}}</td>
...
<td style="width:50%; border: 1px solid #7f7f7f7a; padding:5px;">
  ...
  <strong>{{ucfirst($inspection_items_input->name)}} (Faulty):</strong></td>



<td style="width:50%; border: 1px solid #7f7f7f7a; padding:5px;"> {{ ucfirst($inspection_items_input->reason) }}</td>
      ...
      <!-- <tr>
      <td style="width:50%; padding:5px;"><span ><strong> {{ucfirst($inspection_items_input->name)}} (Faulty):</strong></span>
      </td>
      <td style="width:50%; padding:5px;"><span > {{ ucfirst($inspection_items_input->reason) }} </span>
      </td>
      </tr> -->
      @endif
...
<table style="width:100%; font-family:lato;">
  <tbody>
    <tr style="width:100%;">
      <td style="width:100%;;margin:0;padding:0;display:block; margin-top: 10px;">
      @foreach(explode(',',$details->front_images) as $front_images)
      <a target="_blank" href="<?php echo url('/').'/'.$front_images ?>">
      <img style="margin-top: 10px; width:80px; border: 1px solid rgb(136, 136, 136);  margin-left: 10px;" class="logo" src="<?php echo url('/').'/'.$front_images ?>"></a>
      @endforeach
      </td>
    </tr>
  </tbody>
...
<!-- <table>
  <tbody>
    <tr style="margin:0 0 20px 0; max-width:500px;float:left;">
    @foreach(explode(',',$details->front_images) as $front_images)
    <?php
      ...
      <img style="text-align:center;width:100%;float:left;"  src="<?php echo url('/').'/'.$front_images ?>">
